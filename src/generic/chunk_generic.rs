...
impl<U: UnitTrait> Chunk<U> {
    ...
    pub fn empty() -> Chunk<U> {
        Chunk { frames: Range::empty() }
    }
    ...
    //     forall(|j: usize| (i <= j && j < chunk_list.arr.len()) ==> chunk_list.arr[j].is_none())
    // }))]
    // fn new_pre_heap(chunk_range: T, chunk_list: &mut StaticArray<U>) -> Result<(FrameChunk<U, T>, usize), ChunkCreationError> {
    //     if chunk_range.is_empty() {
    //         return Err(ChunkCreationError::InvalidRange);
    //     }
    ...
    // pub(crate) fn trusted_new(frames: T) -> FrameChunk<U> {
    //     FrameChunk{ frames }
    // }
