...
pub struct Chunk<T: RangeTrait + Copy + PartialEq> {
    frames: T
}

// assert_not_impl_any!(FrameChunk: DerefMut, Clone);

impl<T: RangeTrait + Copy + PartialEq> Chunk<T> {
    #[pure]
    #[trusted]
    #[ensures(result == *self.frames.start())]
    pub fn start(&self) -> RangeTrait::Unit {
        *self.frames.start()
    }
    ...
    pub fn end(&self) -> RangeTrait::Unit {
        *self.frames.end()
    }

    #[ensures(result.is_empty())]
    pub const fn empty() -> Chunk<T> {
        Chunk { frames: T::empty() }
    }
    ...
    fn new(chunk_range: T, chunk_list: &mut List<T>) -> Result<FrameChunk<U, T>, ChunkCreationError> {
    ...
    fn new_pre_heap(chunk_range: T, chunk_list: &mut StaticArray<T>) -> Result<(FrameChunk<U, T>, usize), ChunkCreationError> {
        if chunk_range.is_empty() {
            return Err(ChunkCreationError::InvalidRange);
        }
    ...
    // pub(crate) fn trusted_new(frames: T) -> FrameChunk<T> {
    //     FrameChunk{ frames }
    // }
...
impl<T: RangeTrait + Copy + PartialEq> Deref for FrameChunk<T> {
    type Target = T;
    #[pure]
    fn deref(&self) -> &T {
        &self.frames
