...
pub struct PageChunkAllocator{
    ...
    pub(crate) list: List<PageRange>,
    pub(crate) array: StaticArray<PageRange>
}
...
impl PageChunk {
    ...
    pub const fn empty() -> PageChunk {
        PageChunk { frames: PageRange::empty() }
    }
    ...
    fn new(chunk_range: PageRange, chunk_list: &mut List<PageRange>) -> Result<PageChunk, ChunkCreationError> {
        ...
        if overlap_idx.is_some(){
            ...
            Ok(PageChunk { frames: chunk_range })
        }
    }
    ...
    fn new_pre_heap(chunk_range: PageRange, chunk_list: &mut StaticArray<PageRange>) -> Result<(PageChunk, usize), ChunkCreationError> {
        if chunk_range.is_empty() {
            return Err(ChunkCreationError::InvalidRange);
        }
        ...
        if overlap_idx.is_some(){
            ...
            match chunk_list.push(chunk_range){
                Ok(idx) => Ok((PageChunk { frames: chunk_range }, idx)),
    ...
    pub fn split_range(self, frames_to_extract: PageRange) -> Result<(Option<PageChunk>, PageChunk, Option<PageChunk>), PageChunk> {
        ...
        let before_start = match before {
            Some(x) => Some(PageChunk { frames: x }),
            None => None
        };
    ...
    pub fn split_at(mut self, at_frame: Page) -> Result<(PageChunk, PageChunk), PageChunk> {
        ...
        core::mem::forget(self);   
        Ok((PageChunk{ frames: first }, PageChunk{ frames: second }))
    }
