...
impl List {
    ...
    pub fn lookup(&self, index: usize) -> PageRange {
    ...
    pub fn push(&mut self, elem: PageRange) {
    ...
    pub fn push_unique_with_precond(&mut self, elem: PageRange) -> Result<(),usize> {
    ...
    pub fn pop(&mut self) -> Option<PageRange> {
    ...
    pub(crate) fn elem_overlaps_in_list(&self, elem: PageRange, index: usize) -> Option<usize> {
        if index == self.len() {
            return None;
        }
