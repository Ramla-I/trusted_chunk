...
impl List {
    ...
    pub fn lookup(&self, index: usize) -> FrameRange {
    ...
    pub fn push(&mut self, elem: FrameRange) {
    ...
    pub fn push_unique_with_precond(&mut self, elem: FrameRange) -> Result<(),usize> {
    ...
    pub fn pop(&mut self) -> Option<FrameRange> {
    ...
    pub(crate) fn elem_overlaps_in_list(&self, elem: FrameRange, index: usize) -> Option<usize> {
        if index == self.len() {
            return None;
        }
